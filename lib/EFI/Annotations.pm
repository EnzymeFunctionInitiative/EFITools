...
sub build_annotations {
    ...
    my $specialValueFunc = {
        ...
        "reviewed_description" => sub { my $key = shift; return merge_anno_rows(\@rows, $key, {"" => "NA"}); },
        "hmp_oxygen" => sub { my $key = shift; return merge_anno_rows(\@rows, $key, {"" => "Not Specified"}); },
    ...
    if ($isLegacy) {
        ...
        $specialValueFunc->{superkingdom} = sub { return merge_anno_rows(\@rows, "Superkingdom", {"" => "None"}); };
        ...
        $specialValueFunc->{tax_order} = sub { return merge_anno_rows(\@rows, "TaxOrder"); };
...
sub get_annotation_data {
    ...
    map { $annoData{$_->{name}} = {order => $idx++, display => $_->{display}, is_num => $_->{ssn_num_type} ? 1 : 0, is_list => $_->{ssn_list_type} ? 1 : 0} } grep { $_->{display} ? 1 : 0 } @fields;
...
sub get_annotation_fields {
    ...
    push @fields, {name => "reviewed_description",      field_type => "db",     type_spec => "VARCHAR(255)",    display => "Swissprot Description",         base_ssn => 1,                                                                          json_type_spec => "str",    json_name => "spd"};
    ...
    push @fields, {name => "gn_gene",                   field_type => "db",     type_spec => "VARCHAR(40)",     display => "GN",                            base_ssn => 1,                                                                          json_type_spec => "str",    json_name => "gn"};

    push @fields, {name => "NCBI_IDs",                  field_type => "ssn",                                    display => "NCBI IDs",                      base_ssn => 1,                      ssn_list_type => 1};
    push @fields, {name => "superkingdom",              field_type => "ssn",                                    display => "Superkingdom",                  base_ssn => 1};
    ...
    push @fields, {name => "order",                     field_type => "ssn",                                    display => "Order",                         base_ssn => 1};
    ...
    push @fields, {name => "UniRef90_IDs",              field_type => "ssn",                                    display => "UniRef90 Cluster IDs",                                              ssn_list_type => 1};
    push @fields, {name => FIELD_UNIREF90_CLUSTER_SIZE, field_type => "ssn",                                    display => "UniRef90 Cluster Size",                         ssn_num_type => 1};
    push @fields, {name => "UniRef100_IDs",             field_type => "ssn",                                    display => "UniRef100 Cluster IDs",                                             ssn_list_type => 1};
...
sub is_list_attribute {
    ...
    if (not exists $self->{list_anno}) {
        $self->{list_anno} = {};
        map { $self->{list_anno}->{$_->{name}} = 1; $self->{list_anno}->{$_->{display}} } grep { $_->{ssn_list_type} ? 1 : 0 } get_annotation_data();
    }
...
sub get_attribute_type {
    ...
    if (not $self->{int_attr_types}) {
        $self->{int_attr_types} = {};
        map { $self->{int_attr_types}->{$_} = 1; } grep { $_->{ssn_num_type}  } get_annotation_fields();
    }
