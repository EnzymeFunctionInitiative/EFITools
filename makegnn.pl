...
BEGIN {
    die "The efiest environment must be loaded before running this script" if not exists $ENV{EFIEST};
}
...
$usage=<<USAGE
usage: makegnn.pl -ssnin <filename> -n <positive integer> -nomatch <filename> -gnn <filename> -ssnout <filename>
    ...
    -config         path to config file that specifies the database configuration
USAGE
;
...
#unless($noMatchFile=~/./){
#    die "you must specify and output file for noMatchFilees\n$usage";
#}
#unless($noNeighborFile=~/./){
...
if($stats=~/\w+/){
    ...
    open STATS, ">/dev/null" or die "could not dump stats info to dev null\n";
}
...
my ($noMatchFh, $noNeighborFh);
if ($gnn and $noMatchFile and $noNeighborFile) {
    open($noMatchFh, ">$noMatchFile") or die "cannot write file of non-matching accessions\n";
    open($noNeighborFh, ">$noNeighborFile") or die "cannot write file of accessions without neighbors\n";
} else {
    open($noMatchFh, ">/dev/null");
    open($noNeighborFh, ">/dev/null");
}
...
#      print "add ".scalar(@{${$pfamsearch}{'orig'}{$result}})." to pfam $result cluster $key\n";
            ...
            push @{$pfams{$result}{$key}{'neighlist'}}, @{${$pfamsearch}{'neighlist'}{$result}};
            push @{$pfams{$result}{$key}{'dist'}}, @{${$pfamsearch}{'dist'}{$result}};  
            push @{$pfams{$result}{$key}{'stats'}}, @{${$pfamsearch}{'stats'}{$result}};
...
if ($gnn) {
    ...
    print "\nSkipping writing GNN\n";
}


if ($ssnout) {
    print "write out colored ssn network\n";
    writeColoredSsn($ssnout);
}

if ($idOutputFile) {
    writeIdMapping($idOutputFile);
}

closeClusterMapFiles();
...
sub writeIdMapping {
    ...
    foreach my $nodeId (sort keys %constellations) {
        ...
        print IDMAP "$nodeId\t$clusterId\t", $colors{$clusterId}, "\n";
    }
...
sub closeClusterMapFiles {
    ...
    return if not $dataDir or not -d $dataDir;

    foreach my $key (keys %clusterFh) {
        close($clusterFh{$key});
    }
}
...
sub writeColoredSsn {
    ...
    foreach $node (@nodes){
        ...
        saveNodeToClusterMap($nodeId);
        #print "$node\n";
        $writer->startTag('node', 'id' => $node->getAttribute('label'), 'label' => $node->getAttribute('label'));
        #find color and add attribute
        $writer->emptyTag('att', 'name'=>'node.fillColor', 'type' => 'string', 'value'=> $colors{$numbermatch{$constellations{$nodeId}}});
        $writer->emptyTag('att', 'name'=>'Supercluster', 'type' => 'string', 'value'=> $numbermatch{$constellations{$nodeId}});
...
sub saveNodeToClusterMap {
    ...
    if (not exists $clusterFh{$clusterNum}) {
        open($clusterFh{$clusterNum}, ">$dataDir/cluster_nodes_$clusterNum.txt");
    }

    $clusterFh{$clusterNum}->print("$nodeId\n");
}
