...
my ($domainMapFileName, $mapFileName, $extraRam);
...
my $configFile = $ENV{EFI_CONFIG};


my $workingDir = $ENV{PWD};

$ssnIn = "$workingDir/$ssnIn" if $ssnIn and $ssnIn !~ m/^\//;
...
my $outputPath                  = "$workingDir/$outputDir";
...
$B->addAction("module load $toolModule");
